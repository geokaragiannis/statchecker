...
for run in range(0, k):
    ...
    train_df, test_df = parser.get_test_train_cv_split(lookup_df, "row_index", min_samples=min_samples_per_label)
    print("number of samples after prunning for run number {}: {}".format(run+1, len(train_df) + len(test_df)))
    print("number of unique labels for run number {}: {}".format(run+1, len(train_df.drop_duplicates(subset="row_index"))))
    ...
    y_train = list(train_df["row_index"])
    y_test = list(test_df["row_index"])
...
print("accuracy for {} number of runs for row index {} for topn = {}".format(k, acc, topn))
