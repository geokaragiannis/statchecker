...
def print_stats(df):
    ...
    print("number of row index labels: {}".format(len(df.drop_duplicates(subset="row_index"))))
...
def get_accuracy_from_k_runs(features, labels, topn=5, k=3):
    acc = 0
...
formula_df = formula_df.drop_duplicates(subset=["sent", "claim", "template_formula"]).reset_index(drop=True)
...
f_topn = 5
f_acc_k_runs = get_accuracy_from_k_runs(features_union, f_labels, topn=f_topn)
...
# for lookup predictions

print("Running classifier for lookup index")
l_labels = list(main_df["row_index"])
l_topn = 5
l_acc_k_runs = get_accuracy_from_k_runs(features_union, l_labels, topn=l_topn)
print("accuracy for {} number of runs for lookup index {} for topn = {}".format(k, l_acc_k_runs, l_topn))

# lx_train, lx_test, ly_train, ly_test = get_test_train_split(features_union, l_labels)
#
# l_model = fit_classifier(lx_train, ly_train)
# l_preds, l_acc = get_pred_and_accuracy(l_model, lx_test, ly_test)
# print("accuracy for lookup index {}".format(l_acc))
